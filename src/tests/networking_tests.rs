    ...
    pub fn ut_lind_net_select() {
        ...
        for _counter in 0..600 {
            ...
            for sock in 0..FD_SET_MAX_FD {
                ...
                // If the socket returned was the listener socket, then there's a new conn., so we accept it, and put the client socket in the list of Inputs.
                if sock == serversockfd {
                    ...
                    // If the socket is in an established conn., then we recv the data. If there's no data, then close the client socket.
                    let mut buf = sizecbuf(4);
