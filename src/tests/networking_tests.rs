...
mod fs_tests {
    ...
    pub fn ut_lind_net_bind() {
        ...
        //should work...
        let socket = interface::GenSockaddr::V4(interface::SockaddrV4{ sin_family: 0, sin_port: 50102, sin_addr: interface::V4Addr{ s_addr: 0b01111111000000000000000000000001 }}); //127.0.0.1
    ...
    pub fn ut_lind_net_bind_on_zero() {
        ...
        let socket = interface::GenSockaddr::V4(interface::SockaddrV4{ sin_family: 0, sin_port: 50102, sin_addr: interface::V4Addr{ s_addr: 0 }}); //127.0.0.1
    ...
    pub fn ut_lind_net_bind_multiple() {
        ...
        //allowing port reuse
        assert_eq!(cage.setsockopt_syscall(sockfd, SOL_SOCKET, SO_REUSEPORT, 1), 1);
        assert_eq!(cage.setsockopt_syscall(sockfd2, SOL_SOCKET, SO_REUSEPORT, 1), 1);
    ...
    pub fn ut_lind_net_connect_basic_udp() {
        ...
        let mut socket = interface::GenSockaddr::V4(interface::SockaddrV4{ sin_family: 0, sin_port: 50103, sin_addr: interface::V4Addr{ s_addr: 0b01111111000000000000000000000001 }}); //127.0.0.1
        assert_eq!(cage.connect_syscall(sockfd, &socket), 0);

        //should be able to retarget the socket
        socket = interface::GenSockaddr::V4(interface::SockaddrV4{ sin_family: 0, sin_port: 50104, sin_addr: interface::V4Addr{ s_addr: 0b01111111000000000000000000000001 }}); //127.0.0.1
        assert_eq!(cage.connect_syscall(sockfd, &socket), 0);
    ...
    pub fn ut_lind_net_getsockname() {
        ...
        let mut socket = interface::GenSockaddr::V4(interface::SockaddrV4{ sin_family: 0, sin_port: 50102, sin_addr: interface::V4Addr{ s_addr: 0b01111111000000000000000000000001 }}); //127.0.0.1
        
        assert_eq!(cage.bind_syscall(sockfd, &socket, 4096), 0);
        assert_eq!(cage.getsockname_syscall(sockfd, &mut retsocket), 0);
        assert_eq!(retsocket, socket);    

        socket = interface::GenSockaddr::V4(interface::SockaddrV4{ sin_family: 0, sin_port: 50103, sin_addr: interface::V4Addr{ s_addr: 0b01111111000000000000000000000001 }}); //127.0.0.1
        assert_eq!(cage.bind_syscall(sockfd, &socket, 4096), -(Errno::EINVAL as i32)); //already bound so should fail
        assert_eq!(cage.getsockname_syscall(sockfd, &mut retsocket), 0);
        assert_eq!(retsocket, socket);
