...
void* client(void* v) { 
    ...
    //this unlink fails with error ENOENT, path DNE 
    unlink(CLIENT_PATH);
    ...
    if (flags < 0) {perror("fcntl(F_GETFL)"); return NULL;}
    if (fcntl(client_sock, F_SETFL, flags|O_NONBLOCK) < 0) {perror("fcntl(F_SETFL)"); return NULL;}
    ...
    if ((connret == 0)||(isTCPConnectInProgress))
    {
        ...
        while(1)
        {
            ...
            if (isTCPConnectInProgress) FD_SET(client_sock, &socketsToWatchForWriteReady);
                                    else FD_SET(client_sock, &socketsToWatchForReadReady);
            ...
            else {perror("select()"); return NULL;}
        }
    }
    else perror("connect()");
    close(client_sock);
