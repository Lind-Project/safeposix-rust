...
pub mod fs_tests {
    ...
    fn lock_and_init() -> (Arc<Mutex<FsCalls>>, i32) {
        lindrustinit(0);
        let cage = interface::cagetable_getref(1);
        let shmid = cage.shmget_syscall(31337, 1024, 0666 | IPC_CREAT);
        let shmatret = cage.shmat_syscall(shmid, 0xfffff000 as *mut u8, 0);
        assert_ne!(shmatret, -1);
        let ret_init = cage.sem_init_syscall(shmatret as u32, 1, 1);
        assert_eq!(ret_init, 0);
        let fs_calls = Arc::new(Mutex::new(FsCalls::new(1)));
        let fd = fs_calls.lock().unwrap().open_syscall("/testfile", O_CREAT | O_WRONLY, S_IRWXA);
        (fs_calls, fd)
    }
    
    // Test cases for `dup_syscall`
    
    #[test]
    fn ut_lind_fs_dup_basic() {
        let (fs_calls, fd) = lock_and_init();
        let cage = interface::cagetable_getref(1);
    
        // Duplicate the file descriptor
        let new_fd = fs_calls.lock().unwrap().dup_syscall(fd, None);
    ...
    #[test]
    fn ut_lind_fs_dup_start_desc() {
        let (fs_calls, fd) = lock_and_init();
        let cage = interface::cagetable_getref(1);
    
        // Duplicate the file descriptor with a specific start_desc
        let new_fd = fs_calls.lock().unwrap().dup_syscall(fd, Some(5));
        assert_ne!(new_fd, -(Errno::ENFILE as i32));
        assert_eq!(new_fd, 5);
    ...
    #[test]
    fn ut_lind_fs_dup_existing_start_desc() {
        let (fs_calls, fd) = lock_and_init();
        let cage = interface::cagetable_getref(1);
    
        // Open another file and assign it to file descriptor 5
        let fd2 = fs_calls.lock().unwrap().open_syscall("/testfile2", O_CREAT | O_WRONLY, S_IRWXA);
        assert_ne!(fd2, -(Errno::ENOENT as i32));
        assert_eq!(fs_calls.lock().unwrap().dup2_syscall(fd2, 5), 5);
    
        // Duplicate the first file descriptor with start_desc 5
        let new_fd = fs_calls.lock().unwrap().dup_syscall(fd, Some(5));
        assert_ne!(new_fd, -(Errno::ENFILE as i32));
        assert_eq!(new_fd, 6);
    ...
    fn ut_lind_fs_dup_full_table() {
        ...
        for i in 0..MAXFD {
            let fd = fs_calls.lock().unwrap().open_syscall(&format!("/testfile{}", i), O_CREAT | O_WRONLY, S_IRWXA);
    ...
    // Test cases for `dup2_syscall`
    
    #[test]
    fn ut_lind_fs_dup2_basic() {
        let (fs_calls, fd) = lock_and_init();
        let cage = interface::cagetable_getref(1);
    
        // Duplicate the file descriptor to file descriptor 5
        let new_fd = fs_calls.lock().unwrap().dup2_syscall(fd, 5);
        assert_eq!(new_fd, 5);
    ...
    #[test]
    fn ut_lind_fs_dup2_existing_fd() {
        let (fs_calls, fd) = lock_and_init();
        let cage = interface::cagetable_getref(1);
        ...
        // Duplicate the first file descriptor to file descriptor 5
        let new_fd = fs_calls.lock().unwrap().dup2_syscall(fd, 5);
        assert_eq!(new_fd, 5);
    
        // Verify that the new file descriptor points to the first file
        let mut statdata = StatData::default();
        assert_eq!(cage.fstat_syscall(fd, &mut statdata), 0);
        let mut statdata2 = StatData::default();
        assert_eq!(cage.fstat_syscall(new_fd, &mut statdata2), 0);
    ...
    fn ut_lind_fs_dup2_invalid_fd() {
        let (fs_calls, _) = lock_and_init();
        let cage = interface::cagetable_getref(1);
    
        // Attempt to duplicate an invalid file descriptor
        let new_fd = fs_calls.lock().unwrap().dup2_syscall(-1, 5);
    ...
    #[test]
    fn ut_lind_fs_dup2_out_of_range_fd() {
        let (fs_calls, fd) = lock_and_init();
        let cage = interface::cagetable_getref(1);
    
        // Attempt to duplicate to an out-of-range file descriptor
        let new_fd = fs_calls.lock().unwrap().dup2_syscall(fd, MAXFD + 1);
        assert_eq!(new_fd, -(Errno::EBADF as i32));
