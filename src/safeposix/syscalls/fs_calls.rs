...
impl Cage {
    ...
    pub fn open_syscall(&self, path: &str, flags: i32, mode: u32) -> i32 {
        ...
        match metawalkandparent(truepath.as_path()) {
            ...
            (None, Some(pardirinode)) => {
                ...
                if let Inode::Dir(ref mut ind) = *(self.fs.FS_METADATA.inodetable.get_mut(&pardirinode).unwrap()) {
                ...
                self.log_metadata(&self.fs.FS_METADATA, pardirinode);
                self.log_metadata(&self.fs.FS_METADATA, newinodenum);
            }
            ...
            (Some(inodenum), ..) => {
                ...
                if O_TRUNC == (flags & O_TRUNC) {
                    ...
                    if let Inode::File(ref mut g) = *(self.fs.FS_METADATA.inodetable.get_mut(&inodenum).unwrap()) {
    ...
    pub fn mknod_syscall(&self, path: &str, mode: u32, dev: u64) -> i32 {
        ...
        match metawalkandparent(truepath.as_path()) {
            ...
            (None, Some(pardirinode)) => {
                ...
                let newinodenum = self.fs.FS_METADATA.nextinode.fetch_add(1, interface::RustAtomicOrdering::Relaxed); //fetch_add returns the previous value, which is the inode number we want
                if let Inode::Dir(ref mut parentdir) = *(self.fs.FS_METADATA.inodetable.get_mut(&pardirinode).unwrap()) {
                    parentdir.filename_to_inode_dict.insert(filename, newinodenum);
                    parentdir.linkcount += 1;
                } //insert a reference to the file in the parent directory
                metadata.inodetable.insert(newinodenum, newinode);
        ...
        match metawalk(trueoldpath.as_path()) {
            ...
            Some(inodenum) => {
                ...
                let retval = match metawalkandparent(truenewpath.as_path()) {
                    ...
                    (None, Some(pardirinode)) => {
                        let mut parentinodeobj = self.fs.FS_METADATA.inodetable.get_mut(&pardirinode).unwrap();
                        ...
                        if let Inode::Dir(ref mut parentdirinodeobj) = *parentinodeobj {
                            ...
                            drop(parentinodeobj);
                            self.log_metadata(&self.fs.FS_METADATA, pardirinode);
                            self.log_metadata(&self.fs.FS_METADATA, inodenum);
                        } else {
    ...
    pub fn unlink_syscall(&self, path: &str) -> i32 {
        ...
        match metawalkandparent(truepath.as_path()) {
            ...
            (Some(inodenum), Some(parentinodenum)) => {
                ...
                if !has_domsock {
                    self.log_metadata(&self.fs.FS_METADATA, parentinodenum);
                    self.log_metadata(&self.fs.FS_METADATA, inodenum);
                }
    ...
    pub fn stat_syscall(&self, path: &str, statbuf: &mut StatData) -> i32 {
        ...
        if let Some(inodenum) = metawalk(truepath.as_path()) {
            ...
            statbuf.st_dev = self.fs.FS_METADATA.dev_id;
        ...
        if let Some(filedesc_enum) = &*unlocked_fd {
            ...
            match filedesc_enum {
                File(normalfile_filedesc_obj) => {
                    let inode = self.fs.FS_METADATA.inodetable.get(&normalfile_filedesc_obj.inode).unwrap();
    ...
    pub fn statfs_syscall(&self, path: &str, databuf: &mut FSData) -> i32 {
        ...
        if let Some(inodenum) = metawalk(truepath.as_path()) {
            ...
            databuf.f_fsid = self.fs.FS_METADATA.dev_id;
    ...
    pub fn fstatfs_syscall(&self, fd: i32, databuf: &mut FSData) -> i32 {
        ...
        if let Some(filedesc_enum) = &*unlocked_fd {
            ...
            match filedesc_enum {
                File(normalfile_filedesc_obj) => {
                    let _inodeobj = self.fs.FS_METADATA.inodetable.get(&normalfile_filedesc_obj.inode).unwrap();
        ...
        if let Some(filedesc_enum) = &mut *unlocked_fd {
            ...
            match filedesc_enum {
                ...
                File(ref mut normalfile_filedesc_obj) => {
                    ...
                    let inodeobj = self.fs.FS_METADATA.inodetable.get(&normalfile_filedesc_obj.inode).unwrap();
        ...
        if let Some(filedesc_enum) = &mut *unlocked_fd {
            ...
            match filedesc_enum {
                ...
                File(ref mut normalfile_filedesc_obj) => {
                    ...
                    let mut inodeobj = self.fs.FS_METADATA.inodetable.get_mut(&normalfile_filedesc_obj.inode).unwrap();
                        ...
                        Inode::File(ref mut normalfile_inode_obj) => {
                            ...
                            if let Ok(byteswritten) = fileobject.writeat(buf, count, position) {
                                ...
                                if newposition > normalfile_inode_obj.size {
                                    ...
                                    drop(inodeobj);
                                    self.log_metadata(&self.fs.FS_METADATA, normalfile_filedesc_obj.inode);
	...
	*cwd_container = interface::RustRfc::new(convpath(path_string.as_str()));
    ...
    pub fn _chmod_helper(&self, inodenum: usize, mode: u32) {
         let mut thisinode = self.fs.FS_METADATA.inodetable.get_mut(&inodenum).unwrap();
         let mut log = true;
         if mode & (S_IRWXA|(S_FILETYPEFLAGS as u32)) == mode {
            match *thisinode {
            ...
            drop(thisinode);
            if log { self.log_metadata(&self.fs.FS_METADATA, inodenum) }; 
         }
    }
    ...
    pub fn remove_from_parent_dir(&self, parent_inodenum: usize, truepath: &interface::RustPathBuf) -> i32 {
      if let Inode::Dir(ref mut parent_dir) = *(self.fs.FS_METADATA.inodetable.get_mut(&parent_inodenum).unwrap()) {
            ...
            (Some(inodenum), Some(parent_inodenum)) => {
                ...
                match &mut *inodeobj {
                    ...
                    Inode::Dir(ref mut dir_obj) => {
                        ...
                        if remove_inode { self.fs.FS_METADATA.inodetable.remove(&inodenum).unwrap(); } 

                        self.log_metadata(&self.fs.FS_METADATA, parent_inodenum);
                        self.log_metadata(&self.fs.FS_METADATA, inodenum);       
                        0 // success
                    }
    ...
    pub fn rename_syscall(&self, oldpath: &str, newpath: &str) -> i32 {
        ...
        match metawalkandparent(true_oldpath.as_path()) {
            ...
            (Some(inodenum), Some(parent_inodenum)) => {
                ...
                let pardir_inodeobj = self.fs.FS_METADATA.inodetable.get_mut(&parent_inodenum).unwrap();
