...
pub fn run_benchmark(c: &mut Criterion) {
    ...
    group.bench_function("TF02: Lind write", |b| {
        b.iter(|| {
    ...
    group.bench_function("TF02: Lind read", |b| {
        b.iter(|| {
    ...
    group.bench_function("TF02: Native OS kernel write", |b| {
        b.iter(|| unsafe {
            let _ = libc::write(
                fd,
                tests::str2cbuf("Well, hello there!!!") as *const c_void,
                20,
            );
        })
    });
    ...
    // For comparison let's time the native OS...
    group.bench_function("TF02: Native OS kernel read", |b| {
        b.iter(|| unsafe {
            let mut read_buffer = tests::sizecbuf(20);
